...
variable "amplify_web_url" {
  ...
  default     = "https://dev.gabag-operations-platform.example.com"
}
...
variable "domain_name" {
  ...
  default     = "dev.gabag-operations-platform.example.com"
}

variable "create_dns_zone" {
  type        = bool
  description = "Create a Route53 hosted zone for domain_name. Dev default is false."
  default     = false
}
...
variable "manage_amplify_git" {
  type        = bool
  description = "If false, connect GitHub manually in Amplify Console (recommended for dev)"
  default     = false
}
